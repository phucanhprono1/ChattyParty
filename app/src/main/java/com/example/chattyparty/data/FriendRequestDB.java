...
public class FriendRequestDB extends SQLiteOpenHelper {
    ...
    public void onCreate(SQLiteDatabase sqLiteDatabase) {
        String SQL_CREATE_ENTRIES = "CREATE TABLE IF NOT EXISTS " + FeedEntry.TABLE_NAME1 + " (" +
                FeedEntry.COLUMN_NAME_SENDER + " TEXT PRIMARY KEY," +
                FeedEntry.COLUMN_NAME_RECEIVER + " TEXT," +
                FeedEntry.COLUMN_NAME_ID + " TEXT," +
                FeedEntry.COLUMN_NAME_SENDER_IMAGE + " TEXT," +
                FeedEntry.COLUMN_NAME_SENDER_NAME + " TEXT," +
    ...
    public void addFriendRequest(FriendRequest friendRequest){
        ...
        db.execSQL("INSERT INTO "+FeedEntry.TABLE_NAME1+" VALUES('"+friendRequest.sender+"','"+friendRequest.getReceiver()+"','"+friendRequest.id+"','"+friendRequest.senderImage+"','"+friendRequest.senderName+"','"+friendRequest.idRoom+"','"+friendRequest.name+"','"+friendRequest.email+"','"+friendRequest.avata+"','"+friendRequest.status+"')");
        db.close();
    }
    public void deleteFriendRequest(String id){
        SQLiteDatabase db=this.getWritableDatabase();
        db.execSQL("DELETE FROM "+FeedEntry.TABLE_NAME1+" WHERE "+FeedEntry.COLUMN_NAME_ID+"='"+id+"'");
        db.close();
    }
    public void updateFriendRequest(String id,String status){
        SQLiteDatabase db=this.getWritableDatabase();
        db.execSQL("UPDATE "+FeedEntry.TABLE_NAME1+" SET "+FeedEntry.COLUMN_NAME_STATUS+"='"+status+"' WHERE "+FeedEntry.COLUMN_NAME_ID+"='"+id+"'");
        db.close();
    }
    ...
    public List<FriendRequest> getAllFriendRequest(){
        ...
        Cursor cursor=db.rawQuery("SELECT * FROM "+FeedEntry.TABLE_NAME1,null);
    ...
    public static class FeedEntry implements BaseColumns {
        public static final String TABLE_NAME1 = "friendRequest";
